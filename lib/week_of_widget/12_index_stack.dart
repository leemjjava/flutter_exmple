...
//    animation = Tween(begin: 0.0, end: 1.0).animate(_animationController);
  ...
  Widget build(BuildContext context) {
    ...
    List<Widget> _tabs = [
      MyAnimation(
        animation: animation,
        color: Colors.black,
        child: Text('1 tab', style: whiteText(),),
      ),
      MyAnimation(
        animation: animation,
        child: ListView(
          padding: EdgeInsets.only(bottom: 80),
          children: List.generate(30, (index) => Text('line: $index', style: whiteText(),)).toList(),
        ),
        color: Colors.blue,
      ),
      MyAnimation(
        animation: animation,
        color: Colors.red,
        child: Text('3 tab', style: whiteText(),),
      ),
      MyAnimation(
        animation: animation,
        color: Colors.orange,
        child: Text('4 tab', style: whiteText(),),
      ),
      MyAnimation(
        animation: animation,
        color: Colors.green,
        child: FlatButton(
          ...
          child: Text('5 tab', style: whiteText(),),
        ),
      ),
    ];
  ...
  double getPosition(int index){
    double width = diviceData.size.width - 32;
      ...
      children: List<GestureDetector>.generate(5,(index){
          return GestureDetector(
            child: Container(
              ...
              width: 60,
              height: 60,
              child: Center(child: myIcons[index],),
            ),
            onTap: (){
...
class MyAnimation extends AnimatedWidget {
  MyAnimation({key, animation, this.child, this.color})
      : super(
    key: key,
