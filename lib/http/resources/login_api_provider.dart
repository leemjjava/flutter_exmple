...
class LoginProvide{
  ...
  final RE_TOKEN_KEY = "refresh_token";
  final storage = new FlutterSecureStorage();

  final String _loginURL = "$host/Stage/api/Auth";

  Future<Token> getToken() async {
    Response res = await get(_loginURL,);
    ...
    storage.write(key: TOKEN_KEY, value: token);
  ...
  Future<AuthToken> getAuthToken(String id, String password) async {
    ...
    storage.write(key: TOKEN_KEY, value: accessToken);
    storage.write(key: RE_TOKEN_KEY, value: refreshToken);
  ...
  Future<AuthToken> getReToken() async {
    String refreshToken = await storage.read(key:RE_TOKEN_KEY );
    Map<String, String> headers = {
      RE_TOKEN_KEY : refreshToken,
    };
