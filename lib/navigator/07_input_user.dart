...
class UserInputExState extends State<UserInputEx>{
  ...
  void _mailChanged(bool value) => setState(() => _allowMailing = value);
  void _messageChanged(bool value) => setState(() => _allowMessage = value);

  @override
  void initState() {
    super.initState();
    _progressDialog = getProgressDialog(context, '회원가입 하는 중.....');
  }
  ...
  Widget getMemberCreate(){
    return StreamBuilder<MemberInputResult>(
        ...
        builder: (context, snapshot) {
          if(_progressDialog == null) return Container();
          if(_progressDialog.isShowing() == false) return Container();
